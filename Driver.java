  ...
  public static void main(String[]args){
    if(args.length < 2){
      ...
      if(args.length < 3 || args[2].equals("random")){
        for (int i = 0 ;i < randData.length ; i ++) {randData[i] = new Random().nextInt();}
      }else if(args[2].equals("equal")){
        int thing = new Random().nextInt();
        thing = new Random().nextInt();
        for (int i = 0 ;i < randData.length ; i ++ ) { randData[i] = thing; }
        ...
        Arrays.sort(randData);
        for (int i = 0 ;i < randData.length ; i ++ ) { randData[i] = new Random().nextInt(); }
        int[] newdata = new int[randData.length];
        for (int i = 0 ;i < randData.length ; i ++ ) { newdata[i] = randData[randData.length - i - 1];}
        for (int i = 0 ;i < randData.length ; i ++ ) { randData[i] = newdata[i];}
      }
